		...
		try {
			...
			if (responseData.status === 'error') {
				throw new NodeOperationError(
					context.getNode(),
					`Polycracker Gate Denied: ${String(responseData.message ?? 'Unknown error')}`,
					{ itemIndex: i },
				);
			}
...
async function executeSmart(
	...
	const smartHitUrl = normalizeApiUrlForIpv6(`${baseUrl}/highway/smart-hit`);
	const workflowId = context.getWorkflow().id ?? 'n8n_workflow';
	...
	for (let i = 0; i < items.length; i++) {
		...
		if (!sequenceJwt) {
			throw new NodeOperationError(
				context.getNode(),
				'code_jwt is required: run Polycracker Integration (or upstream node) to provide the sequence baton before Polycracker Smart.',
				{ itemIndex: i },
			);
		}
...
export class Polycracker implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			{
				displayName:
					'First time? <a href="https://portal.polycracker.dev/dashboard?action=register" target="_blank" rel="noopener noreferrer" style="color:#2563eb;font-weight:700;text-decoration:underline">Register for Polycracker</a> to get your User ID and API Key, then add them to your <strong>Polycracker API</strong> credential.',
				name: 'registerNotice',
				type: 'notice',
				typeOptions: {
					clickable: true,
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		if (!baseUrl) {
			throw new NodeOperationError(
				this.getNode(),
				'Polycracker API credentials must include API Base URL.',
			);
		}
